...
func writeButtonWindow(ctx context.Context, out *bytes.Buffer, prompts io.Writer, duration time.Duration) error {
	...
	if errors.Is(err, errNoJabraInputNodes) {
		fmt.Fprintln(out, "  UNAVAILABLE Linux event observation:", err)
	} else if err != nil {
	...
	for {
		...
		case event, ok := <-client.Notifications():
			...
			case "device.button":
				...
				fmt.Fprintf(&out, "  at=%dms IPC event=device.button %s\n", time.Since(started).Milliseconds(), safeControlEvent(event.Params))
			case "sound.changed", "media.action":
				count++
				if count > 64 {
					omitted++
					continue
				}
				fmt.Fprintf(&out, "  at=%dms IPC event=%s (details in service status/history)\n", time.Since(started).Milliseconds(), event.Method)
