...
func runIPC(args []string) error {
	...
	case "capabilities":
		if len(args) != 1 {
			return errors.New("usage: jabridge ipc capabilities")
		}
		var capabilities ipc.ServiceCapabilities
		if err := ipcCall(client, "service.capabilities", nil, &capabilities); err != nil {
			return err
		}
		return printIPCJSON(capabilities)
	case "buttons":
		if len(args) != 1 {
			return errors.New("usage: jabridge ipc buttons")
		}
		var state buttons.Status
		if err := ipcCall(client, "buttons.status", nil, &state); err != nil {
			return err
		}
		return printIPCJSON(state)
	case "ping":
		if len(args) != 1 {
	...
	case "sound":
		if len(args) != 1 {
			return errors.New("usage: jabridge ipc sound")
		}
		var sound pipewire.SoundState
		if err := ipcCall(client, "sound.list", nil, &sound); err != nil {
			return err
		}
		return printIPCJSON(sound)
	case "devices":
		if len(args) != 1 {
