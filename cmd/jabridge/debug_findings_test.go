...
func TestMissingCurrentRuleDoesNotProveDeniedAccess(t *testing.T) {
	steps := strings.Join(reportNextSteps("Current device access rule installed: false"), "\n")
	if !strings.Contains(steps, "missing or different") || strings.Contains(steps, "run jabridge setup") {
		t.Fatal(steps)
	}
}

func TestFirmwareFindingNeverQualifiesEvolve3OrSpeakFlashing(t *testing.T) {
	for _, protocol := range []int{1, 4, 99} {
	...
	for _, protocol := range []int{16, 17} {
		result := firmware.FirmwareDiagnostic{Latest: firmware.LatestInfo{ProductID: 0x253d}, Protocols: []int{protocol}, Cached: true, ChecksumMatches: true}
		if got := nativeFirmwareFinding(result); !strings.Contains(got, "NOT SUPPORTED native archive layout") {
			t.Fatal(got)
		}
		result.NativeLayout = true
		if got := nativeFirmwareFinding(result); !strings.Contains(got, "PASS native archive layout") || !strings.Contains(got, "recovery are NOT TESTED") {
			t.Fatal(got)
		}
	}
	...
	engage := nativeFirmwareFinding(firmware.FirmwareDiagnostic{Latest: firmware.LatestInfo{ProductID: 0x4052}, Protocols: []int{4}, Cached: true, ChecksumMatches: true, NativeLayout: true})
	if strings.Contains(engage, "NOT IMPLEMENTED") || !strings.Contains(engage, "recovery are NOT TESTED") {
		t.Fatal(engage)
	}
