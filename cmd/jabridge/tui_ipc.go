...
func tuiIPCCall(method string, params, result any) error {
	...
	if method == "sound.default" || method == "sound.volume" || method == "sound.mute" || method == "sound.mode" {
	...
	for {
		...
		for connected {
			...
			case event := <-client.Notifications():
				switch event.Method {
				case "device.attached", "device.detached", "device.battery.update", "device.pairing.update", "sound.changed":
...
func syncTUIState(client *ipc.Client) error {
	...
	if err := client.Call(ctx, "sound.list", nil, &sound); err != nil {
		sound = pipewire.SoundState{Error: "Sound unavailable; update and restart the service"}
	}
...
func loadIPCSettings(scope settingScope) ([]menuItem, []deviceSettingValue, error) {
	...
	lines := []menuItem{{id: -1, label: infoLine("Device", device.deviceName)}}
	if scope == settingScopeController {
		lines = append(lines, menuItem{id: -1, label: infoLine("Part", "Link Call Control")})
	}
	if scope != settingScopeDongle {
		lines = append(lines, menuItem{id: -1, label: infoLine("Connection", connection)})
	}
	lines = append(lines, menuItem{id: -1, label: infoLine("USB ID", fmt.Sprintf("0b0e:%04x", device.productID))})
	if scope == settingScopeDongle {
		lines = append(lines,
			menuItem{id: -1, label: infoLine("Firmware", valueOrUnknown(device.firmwareVersion))},
			menuItem{id: -1, label: infoLine("Remembered devices", fmt.Sprint(len(responsePairingList(device))))},
		)
	}
