...
func reportNextSteps(body string) []string {
	...
	switch {
	case strings.Contains(body, connectionIncomplete):
		add("The device scan could not finish. Run debug on the host and check sysfs visibility before deciding that the device is absent or changing device permissions.")
	case strings.Contains(body, connectionNone):
		add("Check the USB plug, cable and port. Run 'lsusb -d 0b0e:' on the host. If using a controller, compare controller alone, headset alone and both connected; tell us which setup you used. No USB entry means we cannot test that connection yet, not proof of a permission problem.")
	case strings.Contains(body, connectionNoHID):
		add("Linux lists a Jabra USB device but no Jabra HID interface. Check the host USB/HID driver and container device visibility; an unsupported interface or reconnect may also explain this. Missing HID nodes are not proof of denied access.")
	}
	if strings.Contains(body, "Current device access rule installed: false") && !strings.Contains(body, connectionNone) {
		add("The bundled access rule is missing or different. This alone does not mean access is blocked; use the actual access results above.")
	}
...
func writeEnvironmentDiagnostic(out *bytes.Buffer) {
	...
	fmt.Fprintln(out, "Sound can show connected Bluetooth audio exposed by PipeWire. That does not enable Bluetooth pairing, headset settings or firmware control.")
}
...
func writeButtonCapabilities(out *bytes.Buffer, paths []string) {
	fmt.Fprintln(out, "\nLinux button capabilities:")
	if len(paths) == 0 {
		fmt.Fprintln(out, "UNAVAILABLE: no Jabra Linux input nodes visible. This can be normal for a device without Linux button events. Check USB/HID detection above; no input node does not mean permission denied.")
	}
	for _, path := range paths {
