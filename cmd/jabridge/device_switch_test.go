...
func TestSelectHeadsetAndDongle(t *testing.T) {
	// Registry tests must not change the developer's real PipeWire defaults.
	oldSnapshot := takeAudioSnapshot
	takeAudioSnapshot = func() (*pipewire.Snapshot, error) { return &pipewire.Snapshot{}, nil }
	t.Cleanup(func() { takeAudioSnapshot = oldSnapshot })
	withDeviceState(t, devices{
