...
import (
	...
	"github.com/Watchdog0x/jabridge/internal/completion"
...
var embeddedUdevRule = assets.UdevRule

var embeddedUserService = assets.UserService

var embeddedBashCompletion = []byte(completion.JabridgeBash)
...
func installUserFiles() (string, error) {
	...
	if err := installBashCompletion(embeddedBashCompletion); err != nil {
		return "", err
	}
...
func installBashCompletion(content []byte) error {
	if len(bytes.TrimSpace(content)) == 0 {
		return errors.New("empty Bash completion")
	}
	homeDirectory, err := os.UserHomeDir()
	if err != nil {
		return fmt.Errorf("find user home: %w", err)
	...
	return installUserFile(completionPath, content, 0o644)
}
