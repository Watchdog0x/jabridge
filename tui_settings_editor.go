...
func saveSettingEditor(results chan<- actionResult) {
	...
	runUIAction(results, editor.setting.label()+" saved and read back from device", func() error {
...
func renderSettingEditor() {
	...
	drawingBox()
	if width < 40 || height < 16 {
		drawCentered(6, "Make the terminal larger to edit", false)
		return
	}
	...
	start := max(0, editor.choice-visible+1)
	for index := start; index < min(len(editor.choices), start+visible); index++ {
	...
	drawSplitActionBar([]string{"Esc/Q Cancel"}, []string{"Up/Down Select", "Enter Save"})
}
