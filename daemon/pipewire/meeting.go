...
func DetectCall(snap *Snapshot) CallState {
	...
	for _, n := range jabraSources {
		if n.Props.MediaClass == "Audio/Source" && strings.EqualFold(n.State, "running") {
			jabraIDs[n.ID] = true
		}
	}
	...
	for _, stream := range snap.StreamNodes() {
		...
		for _, link := range snap.Links {
			if link.State == "active" && link.InputNodeID == stream.ID && jabraIDs[link.OutputNodeID] {
				return CallState{
					InCall:  true,
...
func isCommunicationStream(n Node) bool {
	if n.Props.MediaClass != "Stream/Input/Audio" || !strings.EqualFold(n.State, "running") {
		return false
	}
	...
	if strings.EqualFold(n.Props.MediaRole, "music") {
		return false
	}
	// Known app name match
	app := strings.ToLower(strings.TrimSpace(n.Props.AppName))
	if app != "" && commApps[app] {
		return true
	}
	return false
