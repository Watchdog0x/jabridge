...
func TestInstallVerifiesAndReplacesBinary(t *testing.T) {
	...
	actualArchive := os.Getenv("JABRIDGE_TEST_RELEASE_ARCHIVE")
	if actualArchive != "" {
		archive, err = os.ReadFile(actualArchive)
		if err != nil {
			t.Fatal(err)
		}
	...
	if signaturePath := os.Getenv("JABRIDGE_TEST_RELEASE_SIGNATURE"); signaturePath != "" {
		if actualArchive == "" {
			t.Fatal("release signature requires a release archive")
		}
