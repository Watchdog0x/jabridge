...
type Event struct {
	...
	HIDPage       uint16    `json:"hidPage,omitempty"`
	HIDUsage      uint16    `json:"hidUsage,omitempty"`
	HIDReport     byte      `json:"hidReport,omitempty"`
...
func sanitize(event Event) Event {
	...
	if event.Control == "" || event.HIDPage != 0x0c && event.HIDPage != 0x0b {
		event.HIDPage = 0
		event.HIDUsage = 0
		event.HIDReport = 0
	}
	event.Phase = allowed(event.Phase, "start ok error cancelled observed panic")
	event.Screen = allowed(event.Screen, "home search remembered dongle-settings headset-settings controller-settings devices firmware sound sound-controls sound-volume")
...
func Describe(event Event) string {
	...
	if event.Control != "" {
		text += " control=" + event.Control + " state=" + event.ControlState
	}
	if event.HIDPage != 0 {
		text += fmt.Sprintf(" report=%d page=%04x usage=%04x", event.HIDReport, event.HIDPage, event.HIDUsage)
	}
