_jabridge_completion() {
    ...
    if (( COMP_CWORD == 1 )); then
        mapfile -t COMPREPLY < <(compgen -W 'status battery diagnose settings model models sound use firmware update setup service ipc completion daemon --version --help' -- "$current")
        return
    fi
