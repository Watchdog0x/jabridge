...
func nativeUSBDFUBackend() usbDFUBackend {
	return usbDFUBackend{
		enumerate: enumerateBoundUSB, open: func(d USBDevice) (usbDFUSession, error) { return openDFUUSB(d) },
...
func installUSBDFUChecked(snapshot *firmwareSnapshot, accepted bool, validateTarget func() error) error {
	path := snapshot.path
...
func runUSBDFU(ctx context.Context, original USBDevice, address byte, image *jabraDFUImage, backend usbDFUBackend) (resultErr error) {
	...
	for attempts := 0; attempts < 3; attempts++ {
		...
		record("dfu-verify")
		return verifyUSBDFUAfterFlash(ctx, original, image.Manifest.Version, image.Profile, backend)
	}
	return errors.New("device stayed in DFU runtime after detach")
}

func verifyUSBDFUAfterFlash(parent context.Context, original USBDevice, wanted string, profile usbDFUProfile, backend usbDFUBackend) error {
	ctx, cancel := context.WithTimeout(parent, 45*time.Second)
	defer cancel()
	resetDFU := false
	for {
		device, err := waitUSBDFUDevice(ctx, original.SysPath, func(d USBDevice) bool {
		...
		if profile.runtime(device.ProductID) {
			if profile.runtime(original.ProductID) && (device.ProductID != original.ProductID ||
				(original.Serial != "" && device.Serial != original.Serial)) {
				return errors.New("reconnected USB firmware target does not match the original device identity")
			}
