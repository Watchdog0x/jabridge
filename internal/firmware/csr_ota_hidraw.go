...
func OpenHidraw(path string) (*HidrawTransport, error) {
	...
	descriptor, err := readHidrawReportDescriptorFile(f)
	if err != nil {
		_ = f.Close()
		return nil, fmt.Errorf("inspect management report: %w", err)
	}
	size, ok := parseGnpOutputReportSizeFound(descriptor)
	if !ok {
		_ = f.Close()
		return nil, errors.New("opened device has no supported CSR output report")
	}
...
func (t *HidrawTransport) Read(timeout time.Duration) ([]byte, error) {
	...
	for {
		...
		// A final complete reply can be buffered when the device disconnects.
		// Drain readable data before interpreting HUP as the end of the stream.
		if pollFDs[0].Revents&unix.POLLIN == 0 && pollFDs[0].Revents&(unix.POLLERR|unix.POLLHUP|unix.POLLNVAL) != 0 {
			if pollFDs[0].Revents&unix.POLLHUP != 0 {
				return nil, fmt.Errorf("hidraw disconnected: %w", unix.ENODEV)
			}
...
func readHidrawReportDescriptor(hidrawPath string) ([]byte, error) {
	...
	return readHidrawReportDescriptorFile(f)
}

func readHidrawReportDescriptorFile(f *os.File) ([]byte, error) {
