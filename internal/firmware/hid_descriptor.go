...
func parseHIDReports(descriptor []byte) ([]HIDReport, error) {
	...
	for offset := 0; offset < len(descriptor); {
		...
		if kind == 1 {
			...
			} else if tag == 12 && len(collections) > 0 {
			...
			collectionPage := uint32(0)
			if len(collections) > 0 {
				collectionPage = collections[0]
			}
			fields[k] = append(fields[k], HIDField{collectionPage: collectionPage, OffsetBits: bits[k], SizeBits: state.size, Count: state.count, UsagePage: state.page, Usages: append([]uint32(nil), usages...), UsageMin: usageMin, UsageMax: usageMax, Flags: value, LogicalMin: state.logicalMin, LogicalMax: state.logicalMax})
			bits[k] += count
			usages = nil
			usageMin = 0
			usageMax = 0
